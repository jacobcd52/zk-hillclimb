"""Top-K (Rinberg) capacity rule on the bf16-full-precision-reference dumps.
...
capacity_dump_fp16_{scheme}_seed{seed}.npz (reference = plain bf16 model,
served = M_int argmax). cand_rank is the M_int-served token's rank in the BF16
reference's post-Gumbel ordering, so "top-K" is the true-model verifier's
top-K; the set is scheme-independent here (shared reference).

Outputs:
  capacity/topk_fp16_results_seed{seed}.json
  ../capacity_fp16_topk_ksweep.png
...
LABELS = {
    "baseline": "baseline-native (ref=bf16, served=M_int)",
    "faithful": "faithful-arch-v1 (ref=bf16, served=M_int)",
    "codebook": "codebook (ref=bf16, served=M_int)",
}
...
def main():
    ...
    results = {}
    for sc in SCHEMES:
        dump = Dump(os.path.join(HERE, f"capacity_dump_fp16_{sc}_seed{a.seed}.npz"))
        res = {"scheme": sc, "V": dump.V, "n_positions": dump.P,
               "orientation": "fp16-ref: ref=bf16 full-precision, served=M_int argmax"}
    ...
    ax.set_title("Worst-case covert capacity vs top-K — bf16 full-precision reference\n"
                 "(ref = plain bf16 model, served = M_int; stars = lowest-capacity K; "
                 "dotted = simple-rule min)")
    ax.legend(fontsize=9); ax.grid(alpha=0.3, which="both")
    fig.tight_layout()
    plot_path = os.path.join(OUTDIR, "capacity_fp16_topk_ksweep.png")
    fig.savefig(plot_path, dpi=120); plt.close(fig)

    out = os.path.join(HERE, f"topk_fp16_results_seed{a.seed}.json")
