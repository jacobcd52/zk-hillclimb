#!/bin/bash
# Full regression battery: builds all 24 test binaries (parallel) and runs them
...
# is logUp over towers (section 21.9).
set -u
cd /root/zkllm
NV="nvcc -arch=sm_89 -std=c++17 -O2 -Xcompiler -fopenmp"
TESTS="p3_matmul_selftest p3_rmsnorm_test p3_swiglu_test p3_quant_test p3_bfadd_test p3_rope_test p3_softmax_test p3_gkr_selftest p3_logup_test p3_hawkeye_zk_test zk_gadget_smoke p3_transformer_test zk_layer_smoke p3_transformer_zk_test p3_model_test zk_model_smoke p3_model_zk_test p3_binius_field_test p3_binius_ntt_test p3_binius_pcs_test p3_binius_sumcheck_test p3_binius_e2e_test p3_binius_hawkeye_test p3_binius_logup_test"
echo "=== build ==="
pids=""
for t in $TESTS; do
