#!/bin/bash
# S1+F2 header-edit re-validation: all 13 includer selftests + the wpriv
# batteries (zkob_batchopen wselftest is separate from selftest).
cd /root/zkllm
FAIL=0
run() {
  local name=$1; shift
...
if [ "$FAIL" = "0" ]; then echo "S1F2 REVALIDATION: ALL 14 PASS"; else echo "S1F2 REVALIDATION: FAILURES"; exit 1; fi
