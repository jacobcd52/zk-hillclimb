#!/usr/bin/env python3
# ZKP overhead measured on H100 SXM 80GB: exact-fp8 vs integer-layer prover,
# same Goldilocks/Basefold substrate and soundness. Denominator = saturated
# native bf16 forward on the same H100. vs sequence length and model size.
...
rows = json.load(open('/root/zkllm/bench_h100.json'))
...
for a,(g,xlab,title) in zip(ax, PAN):
    ...
    # fp8 line only where it completes; integer runs the full range
    ...
    dy = 14 if g == 'seq' else -40   # model panel: point sits near the top
    a.annotate('fp8 fails\npast here', (fx[-1], fy[-1]), textcoords='offset points',
               xytext=(6, dy), fontsize=12, color='#c0392b')
...
fig.suptitle('ZKP overhead  (H100)', fontsize=23)
fig.text(0.5, 0.015,
  'overhead  =  time to prove one sequence  ÷  GPU time that sequence costs in a fully-utilized forward pass.\n'
  'Both provers and the forward baseline measured on one H100 SXM 80GB; every proof point is full zero-knowledge and verified.',
  ha='center', fontsize=13.5, color='#444444')
fig.tight_layout(rect=[0.005, 0.075, 1, 0.94])
fig.savefig('/root/overhead_h100.png', dpi=140)
print('wrote /root/overhead_h100.png')
