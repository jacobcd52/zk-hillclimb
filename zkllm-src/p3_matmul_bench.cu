// P3.6 timing: FC matmul prove/verify with GPU encode + GPU Merkle. Reports
// steady-state (CUDA context pre-warmed) wall time + proof size.
#include <cstdio>
#include <vector>
#include <chrono>
...
int main(){
    ...
    printf("=== P3.6 matmul timing  B=%u IN=%u OUT=%u  R=%u Q=%u ===\n",B,IN,OUT,R,Q);
    std::vector<gl_t> X((size_t)B*IN),W((size_t)IN*OUT);
    for(auto&x:X)x=rng()%257; for(auto&x:W)x=rng()%257;
    auto Y=mm(X,W,B,IN,OUT);

    // pre-warm CUDA context (one-time init, excluded from steady-state)
    { std::vector<gl_t> a; p3bf::commit_gpu(W,R,a); cudaDeviceSynchronize(); }

    // cross-check: GPU encode + GPU Merkle == host encode + host Merkle
    { std::vector<gl_t> a,b; p3fri::g_gpu_merkle=false; auto rh=p3bf::commit(W,R,a);
      p3fri::g_gpu_merkle=true; auto rg=p3bf::commit_gpu(W,R,b);
      printf("gpu(encode+merkle) == host commitment: %s\n",(rh==rg)?"YES":"NO"); }

    p3fri::g_gpu_merkle = true;
    auto t0=clk::now(); auto pf=prove(X,W,Y,bb,ii,oo,R,Q,/*gpu=*/true); cudaDeviceSynchronize(); auto t1=clk::now();
    ...
    printf("prove (GPU encode + GPU Merkle + host fold): %7.1f ms\n", ms(t0,t1));
    printf("verify: %7.1f ms\n", ms(t1,t2));
    printf("proof : %7.2f KB\n", bytes/1024.0);
    return ok?0:1;
}
